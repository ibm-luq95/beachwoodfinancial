# -*- coding: utf-8 -*-#
import magic
from django.core.exceptions import ValidationError
from django.template.defaultfilters import filesizeformat
from django.utils.deconstruct import deconstructible


@deconstructible
class FileValidator(object):
    """
    Validates uploaded files based on their size and content type.

    Attributes:
        max_size (int, optional): The maximum allowed file size in bytes.
        min_size (int, optional): The minimum allowed file size in bytes.
        content_types (tuple, optional): A tuple of allowed content types.

    Raises:
        ValidationError: If the file size exceeds the maximum allowed size.
        ValidationError: If the file size is less than the minimum allowed size.
        ValidationError: If the file content type is not supported.

    """

    error_messages = {
        "max_size": (
            "Ensure this file size is not greater than %(max_size)s. Your file size is %(size)s."
        ),
        "min_size": (
            "Ensure this file size is not less than %(min_size)s. Your file size is %(size)s."
        ),
        "content_type": "Files of type %(content_type)s are not supported.",
    }

    def __init__(self, max_size=None, min_size=None, content_types=()):
        """
        Initializes a FileValidator object.

        Args:
            max_size (int, optional): The maximum allowed file size in bytes.
            min_size (int, optional): The minimum allowed file size in bytes.
            content_types (tuple, optional): A tuple of allowed content types.

        """
        self.max_size = max_size
        self.min_size = min_size
        self.content_types = content_types

    def __call__(self, data):
        """
        Validates the uploaded file.

        Args:
            data (File): The uploaded file object.

        Raises:
            ValidationError: If the file size exceeds the maximum allowed size.
            ValidationError: If the file size is less than the minimum allowed size.
            ValidationError: If the file content type is not supported.

        """
        if self.max_size is not None and data.size > self.max_size:
            params = {
                "max_size": filesizeformat(self.max_size),
                "size": filesizeformat(data.size),
            }
            raise ValidationError(self.error_messages["max_size"], "max_size", params)

        if self.min_size is not None and data.size < self.min_size:
            params = {
                "min_size": filesizeformat(self.min_size),
                "size": filesizeformat(data.size),
            }
            raise ValidationError(self.error_messages["min_size"], "min_size", params)

        if self.content_types:
            content_type = magic.from_buffer(data.read(), mime=True)
            data.seek(0)

            if content_type not in self.content_types:
                params = {"content_type": content_type}
                raise ValidationError(
                    self.error_messages["content_type"], "content_type", params
                )

    def __eq__(self, other):
        """
        Checks if two FileValidator objects are equal.

        Args:
            other (FileValidator): The other FileValidator object to compare.

        Returns:
            bool: True if the objects are equal, False otherwise.

        """
        return (
            isinstance(other, FileValidator)
            and self.max_size == other.max_size
            and self.min_size == other.min_size
            and self.content_types == other.content_types
        )
