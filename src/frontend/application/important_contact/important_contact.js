...
function initImportantContactListView() {
  const root = document.getElementById("contactListRoot");
  if (!root) return;

  // 1. Table vs Card Grid View Switcher
  const tableBtn = document.getElementById("contactViewTableBtn");
  const gridBtn = document.getElementById("contactViewGridBtn");
  const tableContainer = document.getElementById("contactTableContainer");
  const gridContainer = document.getElementById("contactCardGridContainer");
  ...
  function setViewMode(mode) {
    if (mode === "grid") {
      ...
      localStorage.setItem("important_contact_view_preference", "grid");
    } else {
      ...
      localStorage.setItem("important_contact_view_preference", "table");
    }
  ...
  const savedPreference =
    localStorage.getItem("important_contact_view_preference") || "table";
  ...
  const toggleFilterBtn = document.getElementById("toggleContactFilterBtn");
  const filterPanel = document.getElementById("contactFilterPanel");
  const filterChevron = document.getElementById("contactFilterChevronIcon");
  ...
  const quickPeekDrawer = document.getElementById("contactQuickPeekDrawer");
  const quickPeekBackdrop = document.getElementById("contactQuickPeekBackdrop");
  const quickPeekDrawerBody = document.getElementById(
    "contactQuickPeekDrawerBody",
  );
  const closeQuickPeekBtn = document.getElementById("closeContactQuickPeekBtn");
  ...
  function openQuickPeek(url) {
    ...
    fetch(url, {
      ...
      .catch((err) => {
        console.error("Contact Quick Peek error:", err);
        quickPeekDrawerBody.innerHTML = `
  ...
  document.addEventListener("click", (e) => {
    const trigger = e.target.closest(".contact-quick-peek-trigger");
