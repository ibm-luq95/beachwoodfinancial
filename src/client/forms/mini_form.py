...
class ClientMiniForm(RemoveFieldsMixin, BWBaseFormMixin):
    ...
    categories = forms.ModelChoiceField(
        queryset=ClientCategory.objects.all(), widget=forms.CheckboxSelectMultiple, required=False
    )
    important_contacts = forms.ModelChoiceField(
        queryset=ImportantContact.objects.all(), widget=forms.CheckboxSelectMultiple, required=False
    )
